...
# Правильный путь импорта для модуля конфигурации
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# Исправляем импорт, убираем telegram_ai_assistant из пути
...
# Добавим больше логирования для отладки
...
async def analyze_message(message_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # If empty message, return basic analysis
    ...
    # Prepare message content for GPT-4o
    content = []
    ...
    # Process attachments if any (images, documents, etc.)
    for attachment in attachments:
        if attachment.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp')):
            try:
                # For image analysis with GPT-4o Vision capabilities
    ...
    try:
        ...
        # Call the OpenAI API with the content
        ...
        # Add original message data for reference
        ...
        # In case of API error, return basic analysis
...
async def extract_task_from_message(message_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Use GPT-4o to extract task details
    system_prompt = """
...
async def detect_question_target(message_data: Dict[str, Any], admin_user_id: int) -> Optional[Dict[str, Any]]:
    ...
    # Use GPT-4o to determine if this is a question directed at the admin
    system_prompt = """
    ...
    try:
        ...
        # If it's a question targeting the admin, return the data
...
async def generate_chat_summary(messages: List[Dict[str, Any]], chat_name: str) -> str:
    ...
    # Use GPT-4o to generate a summary
    system_prompt = """
...
async def analyze_productivity(productivity_data: List[Dict[str, Any]]) -> str:
    ...
    # Format productivity data for the prompt
    formatted_data = []
    ...
    # Use GPT-4o to analyze the productivity
    system_prompt = """
...
async def extract_url_content(url: str) -> str:
    try:
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            # For now, just returning the raw HTML content
            # In a production system, you'd want to parse this with BeautifulSoup
...
async def suggest_response(question_text: str) -> str:
    # Use GPT-4o to generate a helpful response to a question
    system_prompt = """
